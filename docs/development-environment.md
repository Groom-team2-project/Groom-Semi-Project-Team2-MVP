...
`src/main/resources/application.yaml`은 환경 변수를 우선 사용하고, 값이 없으면 로컬 기본값으로 실행됩니다.

```yaml
spring:
  datasource:
    url: ${SPRING_DATASOURCE_URL:jdbc:mysql://localhost:3306/buying_mvp?serverTimezone=Asia/Seoul&characterEncoding=UTF-8}
...
MySQL 테스트 프로필은 `src/test/resources/application-mysql.yaml`을 사용하며, 테스트 DB는 `buying_mvp_test`입니다.

## 테스트 DB 자동 생성

`docker/mysql/init/01-create-test-database.sql`에서 `buying_mvp_test`를 생성합니다.
