...
- **장바구니 주문(checkout) 직후 자동으로 갱신**되어(`cartCheckout` 이 `getMyOrders` 를 다시 호출한다),
  방금 만든 주문이 바로 목록에 나타난다. 마이페이지에 다시 들어올 때도 진입 시 1회 재조회한다.
- 계정이 바뀌면(로그인·토큰 교체·로그아웃) 주문 내역을 즉시 비우고, 늦게 도착한 이전 토큰의
  응답은 버린다. 이전 회원의 주문이 새 계정 화면에 남지 않는다.
